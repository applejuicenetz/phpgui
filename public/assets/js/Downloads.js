...
function dlaction(action) {
    ...
    // Gehe durch alle dl_ids und füge sie zur URL hinzu
    for (var v in dl_ids) {
    ...
    // Wenn der Action-Typ 'settargetdir' ist, frage nach einem neuen Zielverzeichnis
    ...
    // Wenn der Action-Typ 'setpowerdownload' ist, verwende den Wert aus dem Formular
    if (action == 'setpowerdownload')
        dlline += '&action_value=' + document.dl_form.pdl.value;

    // Wenn der Action-Typ 'canceldownload' ist, frage nach einer Bestätigung
    if (action == 'canceldownload' && !confirm(fragetext))
        return;

    // Hier verwenden wir fetch, um den Inhalt dynamisch zu laden, ohne die Seite zu verlassen
    fetch('_ContentLoaded.php' + dlline) // Hole den Inhalt basierend auf der erzeugten URL
        .then(response => response.text()) // Hole den Text der Antwort
        .then(data => {
            // Füge den geladenen Inhalt in das entsprechende Element ein
