...
if (!empty($_GET['clear_list'])) {
    ...
    $template->alert("success", $language->translate('Share.success'), $language->translate('Share.link_export_alert'));
}
...
if (!empty($_GET['shareexpfile'])) {
    ...
    echo '<div class="card mb-4">
            <div class="card-body">
              <h5 class="card-title">' . $language->translate('Share.Link_export_title') . '</h5>';
    ...
    echo "<input type=\"button\" value=\""
        . $language->translate('Share.delet_export') . ""
        . "\" onclick=\"document.location.href='?site=sharefiles&dir=" . $_GET["dir"] . "&clear_list=1&"
        . SID . "';\" />";
    echo "</form>";
                        ...
                        <div class="card">
                            <div class="card-body">
                            	<div class="input-group mb-2">
                            		<button class="btn btn-outline-secondary" type="button" onclick="exportlinks()">' . $language->translate('Share.export') . '</button>
                            	...
                            	<div class="table-responsive">
									<table class="table table-striped">
										<thead>
                							<tr>
                    							<th scope="col">#</th>
                								<th scope="col">' . $language->translate('Share.name') . '</th>
                    							<th width="1" scope="col"><i class="fa fa-info-circle text-info"></i></th>
                    							<th scope="col">' . $language->translate('Share.size') . '</th>
                    							<th width="3" scope="col">' . $language->translate('Share.prio') . '</th>
                    						</tr>
            							</thead>
                						<tbody>';
echo '<tr>
		<th colspan="5" class="align-right">' . $language->translate('System.select') . '
		<input type="button" class="btn btn-outline-primary" value="' . $language->translate('System.all') . '" onclick="selectall();" />
		<input type="button" class="btn btn-outline-danger" value="' . $language->translate('System.none') . '"" onclick="selectnone();" />
		</th>
	</tr>';
//unterverzeichnisse
...
foreach ($list as $shareentry) {
    ...
    ? date("j.n.y - H:i:s", (int)round($shareentry['LASTASKED'] / 1000)) 
    : "N/A";
...
echo strtr($language->translate('Share.prio_spend'), array("%spent" => $Sharelist->spentprio));
