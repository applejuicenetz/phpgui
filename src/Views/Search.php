...
foreach ($links as $link) {
        ...
        if ('file' === $link[1]) {
            ...
            if ($core->command('function', 'processlink?link=' . urlencode($link[0])) == "ok") {
                $template->alert("success", $language->translate('Downloads.get_start'), $text);
            }
                            ...
                            <div class="card-body">
                            	<form action="?site=Search" method="post">
                                	<div class="input-group mb-3">
                                		...
                                		<button class="btn btn-outline-primary" type="submit">' . $language->translate('Search.button') . '</button>
									</div>
								</form>
...
if(!empty($Search->cache['SEARCH'])){
	...
	foreach(array_keys($Search->cache['SEARCH']) as $b){
		...
		if($Search->cache['SEARCH'][$b]['RUNNING']==="true"){
    		...
    		<a class="btn btn-warning" href="?site=search&cancelid=' . $b . '"><i class="fa fa-cross"></i> ' . $language->translate('Search.cancle_search') . '</a>
		</div>';	
		}else{
			$button = '<div class="mb-4">
    		<a class="btn btn-danger" href="?site=search&deleteid=' . $b . '"><i class="fa fa-trash"></i> ' . $language->translate('Search.delet_search') . '</a>
		</div>';
		}
  ...
  <div class="col-sm-9 col-lg-9">
    <div class="tab-content" id="nav-tabContent">
      <div class="tab-pane fade show active" id="search-all" role="tabpanel" aria-labelledby="search-all">
    	<div class="mb-4">
    		<a class="btn btn-danger" href="?site=Search&deleteall=1"><i class="fa fa-trash"></i> ' . $language->translate('Search.delet') . '</a>
		</div>';
...
if(!empty($Search->cache['SEARCHENTRY'])){
	echo'<div class="table-responsive">
			  <table class="table table-striped">
				<tr>
					<th>#</th>
					<th>' . $language->translate('Search.name') . '</th>
					<th>' . $language->translate('Search.size') . '</th>
					<th></th>
	';
	$result_counter=500;
