...
class Server
{
    ...
    function __construct($noload = 0)
    {
        ...
        $language = Language::getLanguage(); // Holt die Instanz der Language-Klasse
        ...
        $this->server_xml['SERVER']['-1']['NAME'] = strtr($language->translate('Server.no_server'), array_flip(get_html_translation_table(HTML_ENTITIES)));
    ...
    function time()
    {
        $timestampInSeconds =$this->server_xml['TIME']['VALUES']['CDATA'] / 1000;
        $formattedDate = date("j.n.y - H:i:s", (int)$timestampInSeconds);
        return $formattedDate;  // Gibt das formatierte Datum aus
    }
